...
if [ "${md5sum_flag}" == 'y' ]; then
  ...
  if [ -e "${oneinstack_file}" ]; then
    now_script_md5=$(md5sum ${oneinstack_file} | awk '{print $1}')
    latest_script_md5=$(curl --connect-timeout 3 -m 5 -s ${mirror_link}/md5sum.txt | grep ${oneinstack_tgz} | awk '{print $1}')
